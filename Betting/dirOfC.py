...
updated = False
tags = makeBank('http://www.realclearpolitics.com/epolls/other/direction_of_country-902.html', 'rcpAvg')
...
while not updated:
    newTags = makeBank('http://www.realclearpolitics.com/epolls/other/direction_of_country-902.html', 'rcpAvg')
    curAvg = newTags[0].getText()
    curAvg = curAvg[index: index + 4]
    if avg != curAvg:
        toText = 'NEW POLL:\nOld dirOfC was ' + avg + '\nNew dirOfC is ' + curAvg
        webbrowser.open('https://www.predictit.org/')
        webbrowser.open('http://www.realclearpolitics.com/epolls/other/direction_of_country-902.html')
